...
if [ -z "$cliphist_list" ]; then
  fuzzel -d --prompt-only "Clipboard empty" --keyboard-focus on-demand
  rm -rf "$thumbnail_dir"
  exit
fi
...
item=$(echo "$cliphist_list" | gawk "$thumbnail" | fuzzel -d --prompt "copy: " --counter --no-sort --with-nth 2 -w 60)
exit_code=$?
